...
MAX_MEMORY = 2**17 # ~131000

def xaiver_init(shape, dtype=None):
    fan_in = shape[1]
    fan_out = shape[0]
    
    return tf.random.normal(shape, mean=0.0, stddev=tf.sqrt(2.0/(fan_in+fan_out)), dtype=dtype)
...
class Model(tf.keras.Model):
    def __init__(self, n_outputs, n_inputs, units=512, *args, **kwargs):
        ...
        self.hidden1 = tf.keras.layers.Dense(units, activation='relu', input_shape=(n_inputs,),
                                             kernel_initializer=xaiver_init)
